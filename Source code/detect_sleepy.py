...
# sleepy_detect入口参数默认值及含义
# EYE_AR_THRESH = 0.2  # 眼睛长宽比
# EYE_AR_CONSEC_FRAMES = 3  # 眨眼闪烁阈值
# MAR_THRESH = 0.5  # 打哈欠嘴巴长宽比
# MOUTH_AR_CONSEC_FRAMES = 6  # 打哈欠闪烁阈值
# HAR_THRESH = 0.34  # 瞌睡点头阈值，约20°左右
# NOD_AR_CONSEC_FRAMES = 3
def sleepy_detect(EYE_AR_THRESH=0.2, EYE_AR_CONSEC_FRAMES=3, MAR_THRESH=0.65, MOUTH_AR_CONSEC_FRAMES=5, HAR_THRESH=5.0,
                  NOD_AR_CONSEC_FRAMES=3):
    print("------已经开始疲劳检测----------------")
    # 初始化帧计数器和眨眼总数
    eCOUNTER = 0
    ...
    while True:
        ...
        for rect in rects:
            ...
            # 第八步：将脸部特征信息转换为numpy中数组array的格式
            ...
            # 第十步：构造函数计算左右眼的EAR值，使用平均值作为最终的EAR;计算打哈欠的MAR值
            ...
            # 第十三步：循环，满足EAR值>0.2条件的，眨眼次数+1
            if ear < EYE_AR_THRESH:  # 眼睛长宽比：0.2
                eCOUNTER += 1
            else:
                # 如果连续3帧都小于阈值，则表示进行了一次较疲劳的眨眼活动
                if eCOUNTER >= EYE_AR_CONSEC_FRAMES:  # 阈值：3
                    eTotal += 1
                    say("检测到一次疲劳眨眼，请注意自己的疲劳状态！")
                    cv2.putText(frame, "Tired Wink!", (100, 150), cv2.FONT_HERSHEY_SIMPLEX, 1, (255, 165, 0), 2)
                # 重置眼帧计数器
                eCOUNTER = 0
            ...
            if mar > MAR_THRESH:  # 张嘴阈值0.5
                mCOUNTER += 1
                cv2.putText(frame, "Yawning!", (100, 200), cv2.FONT_HERSHEY_SIMPLEX, 1, (255, 165, 0), 2)
            elif mar < 0.45:
                ...
                mCOUNTER = 0
            # print("mar=",mar)
            # 第十四步：进行画图操作，同时使用cv2.putText将关键数值量进行显示
            ...
            pre_pitch = har
            if pitch_diff > HAR_THRESH:  # 点头阈值0.3
                hCOUNTER += 1
            else:
                # 如果连续3帧都大于阈值，则表示瞌睡点头一次
                if hCOUNTER >= NOD_AR_CONSEC_FRAMES:  # 阈值：3
                    hTotal += 1
                    cv2.putText(frame, "Nodding!", (100, 250), cv2.FONT_HERSHEY_SIMPLEX, 1, (255, 165, 0), 2)
                    say("检测到瞌睡点头，请立即停车休息！")
                # 重置点头帧计数器
                hCOUNTER = 0
        ...
        if eTotal >= 20 or mTotal >= 5 or hTotal >= 3:
            cv2.putText(frame, "SLEEP!!!", (100, 300), cv2.FONT_HERSHEY_SIMPLEX, 1.4, (0, 255, 255), 3)
...
if __name__ == '__main__':
    # detect_thread = threading.Thread(target=sleepy_detect(), args=())  # args可以给run传参
    detect_thread = threading.Thread(target=sleepy_detect, args=())
    detect_thread.start()
